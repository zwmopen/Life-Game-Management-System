...
export const helpContent: Record<string, HelpContentItem> = {
  ...
  shop: {
    ...
    underlyingPrinciple: '基于电商平台的商品分类和管理逻辑设计，将商品分为实物类和权益/休闲类，方便用户根据需求进行筛选和管理。结合游戏化元素，通过商品分类和管理功能，增强用户的游戏体验和参与感。',
    coreRules: '商品分为实物类和权益/休闲类两大类；实物类商品购买后永久拥有，存入军械库；权益/休闲类商品购买即消耗，代表获得一次享受的权利；金币仅通过完成任务获得；商品分类可通过分类按钮进行筛选；支持商品的添加、编辑和删除操作（仅管理模式）。',
    usageMethods: '1. 查看商品分类与管理模块了解当前商品状态；2. 点击分类按钮筛选对应分类的商品；3. 点击商品卡片查看商品详情；4. 购买商品消耗金币，获得相应的商品或权益；5. 管理模式下可添加、编辑和删除商品；6. 结合任务管理系统，通过完成任务获得金币购买商品。',
