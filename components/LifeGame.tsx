  ...
  useEffect(() => {
    const checkAndFixImages = async () => {
      ...
      const fixedInventory = await checkAndFixProductImages(initialInv.map(item => ({
        ...item,
        type: item.type as ProductType, // 将字符串转换为ProductType枚举
        category: item.category as ProductCategory // 将字符串转换为ProductCategory枚举
      })));
                        ...
                        {manageTaskTab === 'daily' && (
                            <div className={`p-3 border border-dashed rounded mb-4 ${isNeomorphic ? (theme === 'neomorphic-dark' ? 'bg-[#1e1e2e] shadow-[5px_5px_10px_rgba(0,0,0,0.4),-5px_-5px_10px_rgba(30,30,46,0.8)] border-none rounded-lg' : 'bg-[#e0e5ec] shadow-[5px_5px_10px_rgba(163,177,198,0.6),-5px_-5px_10px_rgba(255,255,255,1)] border-none rounded-lg') : (isDark ? 'border-zinc-700 bg-zinc-900/50' : 'border-slate-300 bg-slate-50')}`}>
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                                    ...
                                    <div><label className={`text-[10px] ${isDark ? 'text-zinc-500' : 'text-slate-500'}`}>预估时长</label><input type="number" value={newTaskDuration} onChange={e => setNewTaskDuration(e.target.value)} className={`w-full py-1 outline-none ${textMain} ${isNeomorphic ? (theme === 'neomorphic-dark' ? 'bg-[#1e1e2e] shadow-[inset_2px_2px_4px_rgba(0,0,0,0.4),inset_-2px_-2px_4px_rgba(30,30,46,0.8)] border-none rounded-lg p-2' : 'bg-[#e0e5ec] shadow-[inset_2px_2px_4px_rgba(163,177,198,0.6),inset_-2px_-2px_4px_rgba(255,255,255,1)] border-none rounded-lg p-2') : (isDark ? 'bg-transparent border-zinc-700' : 'bg-transparent border-slate-300')}`} /></div>
                                </div>
                                <button 
                                    onClick={() => {
                            ...
                            <div className="space-y-4">
                                ...
                                <div className={`p-3 border border-dashed rounded ${isNeomorphic ? (theme === 'neomorphic-dark' ? 'bg-[#1e1e2e] shadow-[5px_5px_10px_rgba(0,0,0,0.4),-5px_-5px_10px_rgba(30,30,46,0.8)] border-none rounded-lg' : 'bg-[#e0e5ec] shadow-[5px_5px_10px_rgba(163,177,198,0.6),-5px_-5px_10px_rgba(255,255,255,1)] border-none rounded-lg') : (isDark ? 'border-zinc-700 bg-zinc-900/50' : 'border-slate-300 bg-slate-50')}`}>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                                        ...
                                        <div><label className={`text-[10px] ${isDark ? 'text-zinc-500' : 'text-slate-500'}`}>经验奖励</label><input type="number" id="randomXP" value="30" className={`w-full py-1 outline-none ${textMain} ${isNeomorphic ? (theme === 'neomorphic-dark' ? 'bg-[#1e1e2e] shadow-[inset_2px_2px_4px_rgba(0,0,0,0.4),inset_-2px_-2px_4px_rgba(30,30,46,0.8)] border-none rounded-lg p-2' : 'bg-[#e0e5ec] shadow-[inset_2px_2px_4px_rgba(163,177,198,0.6),inset_-2px_-2px_4px_rgba(255,255,255,1)] border-none rounded-lg p-2') : (isDark ? 'bg-transparent border-zinc-700' : 'bg-transparent border-slate-300')}`} /></div>
                                    ...
                                    <button 
                                        onClick={() => {
                                            if(newChallengeText.trim()) {
                                                ...
                                                const xp = parseInt((document.getElementById('randomXP') as HTMLInputElement).value) || 30;
                                                ...
                                                const newTask = {
                                                    ...
                                                    xp: xp,
                                                    duration: duration,
                                                    attr: AttributeType.WEALTH, // 默认设置为财富属性
                                                ...
                                                (document.getElementById('randomXP') as HTMLInputElement).value = '30';
                                            }
                                        }} 
                                                            ...
                                                            <div key={index} className={`flex items-center justify-between p-2 rounded ${isNeomorphic ? (theme === 'neomorphic-dark' ? 'bg-[#1e1e2e] shadow-[5px_5px_10px_rgba(0,0,0,0.4),-5px_-5px_10px_rgba(30,30,46,0.8)] border-none rounded-lg' : 'bg-[#e0e5ec] shadow-[5px_5px_10px_rgba(163,177,198,0.6),-5px_-5px_10px_rgba(255,255,255,1)] border-none rounded-lg') : (isDark ? 'border-zinc-800 bg-zinc-900/50' : 'border-slate-200 bg-slate-50')}`}>
                                                                ...
                                                                <div className="flex gap-2">
                                                                    <button 
                                                                        onClick={() => {
                                                                            // 编辑任务
                                                                            setNewChallengeText(task.text);
                                                                            setEditingTaskId(task.id); // 设置编辑任务ID
                                                                            ...
                                                                            const xpInput = document.getElementById('randomXP') as HTMLInputElement;
                                                                            
                                                                            if (categorySelect) categorySelect.value = task.category || 'health';
                                                                            if (rewardInput) rewardInput.value = (task.gold || 20).toString();
                                                                            if (durationInput) durationInput.value = (task.duration || 20).toString();
                                                                            if (xpInput) xpInput.value = (task.xp || 30).toString();
                                                                            
                                                                            setIsAddTaskOpen(true); // 打开编辑任务模态框
                                                                        }}
