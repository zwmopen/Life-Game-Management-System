# 商品分类标准

## 1. 概述

本文档定义了人生游戏管理系统中商品的标准分类体系，确保商品数据的统一性和可维护性。

---

## 2. 商品分类体系

### 2.1 主分类列表

| 分类名称 | 说明 | 商品类型 |
|----------|------|----------|
| 数码 | 数码产品、电子设备 | physical |
| 家居 | 家居用品、家具、房产、车辆 | physical |
| 形象设计与穿搭 | 服装、配饰、美容美发 | physical |
| 吃喝 | 食品、饮料、餐饮 | leisure |
| 饮食 | 食品、饮料（备用分类） | leisure |
| 休闲娱乐 | 娱乐活动、票务、旅游 | leisure, rights |
| 会员充值 | 会员服务、充值卡、社群 | rights |

### 2.2 商品类型说明

| 类型 | 说明 | 示例 |
|------|------|------|
| `physical` | 实体商品，可持有 | 手机、衣服、家具 |
| `leisure` | 休闲消费，一次性体验 | 餐饮、娱乐活动 |
| `rights` | 权益类商品，具有时效性 | 会员、票务、充值卡 |

---

## 3. 商品ID命名规范

### 3.1 ID格式

```
{类型前缀}_{分类缩写}_{序号}
```

### 3.2 类型前缀

| 前缀 | 商品类型 |
|------|----------|
| `p_` | physical（实体商品） |
| `s_` | leisure（休闲消费） |
| `r_` | rights（权益类） |

### 3.3 分类缩写

| 缩写 | 分类名称 |
|------|----------|
| `dig` | 数码 |
| `gear` | 装备 |
| `home` | 家居 |
| `cloth` | 服装 |
| `food` | 饮食 |
| `ent` | 娱乐 |
| `hair` | 理发 |
| `spa` | 按摩 |
| `books` | 书籍 |
| `tick` | 票务 |
| `vip` | 会员 |
| `char` | 充值 |
| `misc` | 其他 |
| `sport` | 运动 |
| `gift` | 礼物 |
| `property` | 房产 |
| `car` | 车辆 |
| `service` | 服务 |

### 3.4 示例

```
p_dig_1      // 实体商品-数码-1（iPhone 16 Pro）
s_food_1     // 休闲消费-饮食-1（辣条一包）
r_vip_1      // 权益类-会员-1（网易云VIP）
p_home_1     // 实体商品-家居-1（室内炉锅桌子）
p_cloth_1    // 实体商品-服装-1（衣服一件）
```

---

## 4. 商品数据结构

### 4.1 ShopItem 接口

```typescript
interface ShopItem {
  id: string;           // 商品ID，唯一标识
  name: string;         // 商品名称
  description: string;  // 商品描述
  cost: number;         // 价格（金币）
  type: 'physical' | 'leisure' | 'rights';  // 商品类型
  owned: boolean;       // 是否已拥有
  icon: React.ReactNode; // 图标组件
  category: string;     // 分类名称
  image: string;        // 商品图片URL
}
```

### 4.2 图标规范

- 使用 `lucide-react` 图标库
- 图标尺寸统一为 `size={24}`
- 颜色使用 Tailwind CSS 类名，如 `text-blue-500`

### 4.3 图片规范

- 使用 Unsplash 图片
- 尺寸参数：`w=400&h=400&fit=crop`
- URL格式：`https://images.unsplash.com/photo-{id}?w=400&h=400&fit=crop`

#### 图片ID获取步骤（重要！）

**必须从Unsplash网站获取真实的图片ID，不能随意构造！**

1. **访问 Unsplash**：https://unsplash.com
2. **搜索关键词**：输入商品相关的英文关键词，如 `pillow`、`pet food`、`headphones`
3. **选择图片**：点击喜欢的图片进入详情页
4. **获取图片ID**：
   - 从浏览器地址栏获取URL，如：`https://unsplash.com/photos/abc123xyz`
   - 图片ID就是 `abc123xyz` 部分
5. **拼接完整链接**：`https://images.unsplash.com/photo-abc123xyz?w=400&h=400&fit=crop`

#### 常见错误

❌ **错误做法**：随意编造图片ID
```
https://images.unsplash.com/photo-1595376685484-8c7a3-228ff5388ef8?w=400&h=400&fit=crop
（这个ID是编造的，图片无法访问）
```

✅ **正确做法**：使用从Unsplash网站获取的真实ID
```
https://images.unsplash.com/photo-1584100936595-c0654b55a2e2?w=400&h=400&fit=crop
（这个ID来自真实的Unsplash图片）
```

#### 验证图片链接

添加新商品后，应在浏览器中打开图片链接验证是否可访问。

---

## 5. 分类详细说明

### 5.1 数码

| 商品示例 | 价格范围 | 说明 |
|----------|----------|------|
| iPhone 16 Pro | 8999 | 顶级通讯终端 |
| MacBook Pro M4 | 16000 | 生产力核心武器 |
| 降噪耳机 | 2000 | 主动降噪 |
| 机械键盘 | 800 | 输入体验升级 |
| 大疆pocket 3 | 2590 | 便携稳定器 |
| iPad | 3299 | 平板电脑 |
| 扫地机器人 | 7200 | 智能清扫 |
| 戴森吹风机 | 1999 | 高端吹风机 |

### 5.2 家居

| 商品示例 | 价格范围 | 说明 |
|----------|----------|------|
| 人体工学椅 | 1500 | 脊椎防御系统 |
| 乳胶枕头 | 300 | 深度睡眠加速器 |
| 房子一间 | 500000 | 温馨小屋 |
| 理想汽车 | 250000 | 新能源汽车 |
| 厨房套装 | 3500 | 全套厨房设备 |

### 5.3 形象设计与穿搭

| 商品示例 | 价格范围 | 说明 |
|----------|----------|------|
| 新战靴 | 800 | 行动力+10% |
| 防蓝光眼镜 | 400 | 护眼Buff |
| 衣服一件 | 299 | 时尚服装 |
| 素颜霜 | 100 | 提升气色 |
| 头发烫染 | 100 | 时尚发型 |

### 5.4 吃喝

| 商品示例 | 价格范围 | 说明 |
|----------|----------|------|
| 辣条一包 | 1 | 廉价多巴胺 |
| 快乐水 | 5 | 瞬间恢复心情 |
| 烧烤 | 60 | 街头美食 |
| 烤鱼 | 100-188 | 美味烤鱼 |
| 酱骨头套餐 | 158 | 酱骨头套餐 |

### 5.5 休闲娱乐

| 商品示例 | 价格范围 | 说明 |
|----------|----------|------|
| 看小说 | 30-60 | 沉浸式阅读 |
| 刷短视频 | 30-60 | 短平快娱乐 |
| 理发 | 48 | 魅力值回升 |
| 按摩 | 198-200 | 缓解疲劳 |
| 电影票 | 45 | 娱乐放松 |
| 演唱会门票 | 498 | 音乐盛宴 |
| 旅游 | 298-50000 | 探索新地图 |

### 5.6 会员充值

| 商品示例 | 价格范围 | 说明 |
|----------|----------|------|
| 网易云VIP | 15 | 听觉享受 |
| 健身会员 | 298 | 健身特权 |
| 社群门票 | 1000-20000 | 高端社群 |
| 话费充值卡 | 50-99 | 通讯保障 |

---

## 6. 添加新商品流程

1. 确定商品分类和类型
2. 按命名规范生成ID
3. 准备商品信息（名称、描述、价格）
4. 选择合适的图标和图片
5. 在 `constants/shopCatalog.tsx` 中添加商品数据
6. 验证商品显示正常

---

## 8. 商品卡片设计规范

### 8.1 卡片整体布局

商品卡片采用**图片背景+渐变遮罩+信息叠加**的设计模式：

```
┌─────────────────────────────┐
│ [商品图片 - 全背景]           │
│                             │
│    ┌─────────────────────┐  │
│    │ 渐变遮罩 (从浅到深)   │  │
│    │                     │  │
│    │   ¥价格 (胶囊样式)   │  │
│    │   商品名称           │  │
│    │   商品描述           │  │
│    │   [购买按钮]         │  │
│    └─────────────────────┘  │
└─────────────────────────────┘
```

### 8.2 卡片容器样式

| 属性 | 值 | 说明 |
|------|-----|------|
| 布局 | `grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6` | 响应式网格布局 |
| 间距 | `gap-4` | 卡片间距16px |
| 最小高度 | `minHeight: '280px'` | 卡片最小高度 |
| 圆角 | `rounded-xl` | 12px圆角 |
| 过渡 | `transition-all duration-300` | 300ms过渡动画 |

### 8.3 拟态风格样式

#### 深色模式（neomorphic-dark）

```css
背景色: bg-[#1e1e2e]
边框色: border-[#1e1e2e]
阴影: shadow-[8px_8px_16px_rgba(0,0,0,0.4),-8px_-8px_16px_rgba(30,30,46,0.8)]
悬停阴影: hover:shadow-[10px_10px_20px_rgba(0,0,0,0.5),-10px_-10px_20px_rgba(30,30,46,1)]
激活阴影: active:shadow-[inset_5px_5px_10px_rgba(0,0,0,0.4),inset_-5px_-5px_10px_rgba(30,30,46,0.8)]
```

#### 浅色模式（neomorphic-light）

```css
背景色: bg-[#e0e5ec]
边框色: border-[#e0e5ec]
阴影: shadow-[8px_8px_16px_rgba(163,177,198,0.6),-8px_-8px_16px_rgba(255,255,255,1)]
悬停阴影: hover:shadow-[10px_10px_20px_rgba(163,177,198,0.7),-10px_-10px_20px_rgba(255,255,255,1)]
激活阴影: active:shadow-[inset_5px_5px_10px_rgba(163,177,198,0.6),inset_-5px_-5px_10px_rgba(255,255,255,1)]
```

### 8.4 商品图片样式

| 属性 | 值 | 说明 |
|------|-----|------|
| 定位 | `absolute top-0 left-0 w-full h-full` | 全背景覆盖 |
| 对象适应 | `object-cover` | 保持比例裁剪 |
| 悬停效果 | `group-hover:scale-105` | 悬停放大5% |
| 过渡 | `transition-all duration-500` | 500ms过渡 |

#### 图片链接规范

**图片来源：**
- 使用 Unsplash 图片服务
- URL格式：`https://images.unsplash.com/photo-{id}?w=400&h=400&fit=crop`
- 尺寸参数：`w=400&h=400&fit=crop`（400x400像素，裁剪适配）

**图片加载逻辑：**
```typescript
// 只显示有效的 images.unsplash.com 链接
{item.image && item.image.includes('images.unsplash.com') ? (
  <img src={item.image} alt={item.name} ... />
) : (
  // 没有有效图片时，显示默认图标
  <div className="fallback-icon">{item.icon}</div>
)}
```

**图片加载失败处理：**
- 图片加载失败时自动隐藏
- 显示 fallback 图标（商品 icon 字段）
- fallback 背景色：深色 `#1a1a2e`，浅色 `#e0e5ec`

**示例图片URL：**
```
https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=400&h=400&fit=crop  // 手机
https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=400&h=400&fit=crop  // 耳机
https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=400&h=400&fit=crop  // 手表
```

### 8.5 渐变遮罩样式

渐变遮罩从商品图片顶部70px处开始，向下渐变：

**深色模式：**
```css
background: linear-gradient(to bottom, 
  rgba(30,30,46,0) 0%, 
  rgba(30,30,46,0.5) 20%, 
  rgba(30,30,46,0.8) 50%, 
  rgba(30,30,46,0.95) 100%)
```

**浅色模式：**
```css
background: linear-gradient(to bottom, 
  rgba(255,255,255,0) 0%, 
  rgba(255,255,255,0.3) 30%, 
  rgba(255,255,255,0.6) 60%, 
  rgba(255,255,255,0.8) 100%)
```

### 8.6 价格标签样式

| 属性 | 深色模式 | 浅色模式 |
|------|----------|----------|
| 背景 | `bg-yellow-600/30` | `bg-[#fff3cd]` |
| 文字 | `text-yellow-400` | `text-[#fd7e14]` |
| 边框 | `border border-yellow-600/50` | 无 |
| 圆角 | `rounded-full` | `rounded-full` |
| 内边距 | `px-4 py-1.5` | `px-4 py-1.5` |
| 字体 | `text-sm font-bold` | `text-sm font-bold` |

### 8.7 商品名称样式

| 属性 | 值 |
|------|-----|
| 字体 | `font-bold text-xl` |
| 颜色 | `text-white` |
| 文字阴影 | `text-shadow: 0 1px 2px rgba(0,0,0,0.8)` |
| 换行 | `break-words` |

### 8.8 商品描述样式

| 属性 | 深色模式 | 浅色模式 |
|------|----------|----------|
| 字体 | `text-sm` | `text-sm` |
| 颜色 | `text-white/90` | `text-zinc-700` |
| 行数限制 | `line-clamp-2` | `line-clamp-2` |
| 文字阴影 | `0 1px 2px rgba(0,0,0,0.8)` | `0 1px 2px rgba(255,255,255,0.8)` |

### 8.9 购买按钮样式

| 状态 | 样式 |
|------|------|
| 默认（深色） | `bg-gradient-to-r from-blue-600 to-purple-600 text-white` |
| 默认（浅色） | `bg-gradient-to-r from-blue-500 to-purple-500 text-white` |
| 悬停 | `hover:from-blue-500 hover:to-purple-500` |
| 已拥有 | `bg-zinc-800/30 text-zinc-500` |
| 圆角 | `rounded-full` |
| 内边距 | `px-8 py-2.5` |
| 字体 | `text-sm font-bold` |

### 8.10 管理模式样式

管理模式下，卡片边框变为红色虚线：

```css
border-red-500/30
```

编辑/删除按钮位于右上角，使用圆形拟态按钮：

| 按钮 | 颜色 |
|------|------|
| 编辑 | `text-blue-400`（深色）/ `text-blue-600`（浅色） |
| 删除 | `text-red-400`（深色）/ `text-red-600`（浅色） |

### 8.11 盲盒卡片样式

盲盒卡片采用居中布局：

| 属性 | 值 |
|------|-----|
| 图标 | `text-4xl mb-2` 🎁 |
| 价格 | `text-lg font-bold` |
| 描述 | `text-xs mt-1` |
| 购买按钮 | `w-full py-1 text-[12px] font-bold rounded-md` |

---

## 9. 注意事项

- 商品价格应合理，符合现实逻辑
- 图标颜色应与分类主题协调
- 图片URL确保可访问
- 分类名称保持一致，避免重复定义

---

## 9. 常见问题与解决方案

### 9.1 分类名称不一致导致商品显示为0

**问题描述：**
修改商品的 `category` 字段后，点击分类按钮时显示0个商品。

**原因分析：**
1. 商品的 `category` 字段已更新为新名称（如"家居数码"、"个人形象"）
2. 但多个地方的分类列表还是旧名称（如"数码"、"家居"、"形象设计与穿搭"）
3. 点击分类按钮时，系统筛选 `category === 按钮名称` 的商品
4. 由于名称不匹配，找不到商品，显示为0

**涉及文件：**
- `components\ShopCatalog\index.tsx` - 第42行，groups默认值
- `hooks\useShop.ts` - 第30行，groups初始值
- `types.ts` - 第15-23行，ProductCategory枚举
- `components\HelpSystem\HelpContent.tsx` - 多处分类说明

**解决方案：**
1. **统一分类名称**：修改商品 `category` 字段时，同步更新所有相关文件
2. **检查清单**：
   - [ ] `constants/shopCatalog.tsx` - 商品数据的 `category` 字段
   - [ ] `components/ShopCatalog/index.tsx` - groups默认值
   - [ ] `hooks/useShop.ts` - groups初始值
   - [ ] `types.ts` - ProductCategory枚举
   - [ ] `components/HelpSystem/HelpContent.tsx` - 帮助文档中的分类说明

3. **预防措施**：
   - 建议使用常量或枚举统一管理分类名称
   - 避免在多个地方硬编码相同的分类列表
   - 修改分类名称时，全局搜索替换所有相关引用

**示例：**
```typescript
// ❌ 错误做法：多处硬编码
const groups1 = ['数码', '家居', '形象设计与穿搭'];
const groups2 = ['数码', '家居', '形象设计与穿搭'];

// ✅ 正确做法：使用统一常量
const CATEGORIES = {
  DIGITAL_HOME: '家居数码',
  PERSONAL_IMAGE: '个人形象',
  // ...
} as const;
```

---

## 10. 文档版本

| 版本 | 更新时间 | 更新内容 |
|------|----------|----------|
| 1.0 | 2026-01-10 | 初始版本 |
| 1.1 | 2026-01-21 | 更新分类体系，添加实际商品示例，更新ID命名规范 |
| 1.2 | 2026-02-21 | 新增商品卡片设计规范，包括卡片布局、拟态样式、渐变遮罩、价格标签等详细规范 |
| 1.3 | 2026-02-21 | 补充图片链接规范，包括图片来源、加载逻辑、失败处理和示例URL |
| 1.4 | 2026-02-21 | 新增图片ID获取步骤，强调必须从Unsplash网站获取真实ID，不能随意构造 |
| 1.5 | 2026-02-23 | 新增常见问题与解决方案，包括分类名称不一致导致商品显示为0的问题及解决方案 |

---

**版权所有 © 2026 AI编程 人生游戏管理系统**
