...
export const INITIAL_HABITS = [
  {
    ...
    note: '1. 保持平躺闭眼，在脑海预演 ZWM 2.0 今日全流程：起床、任务推进、项目执行、输出分享，沉浸式感受「已完成目标」的从容与掌控感\n2. 默念肯定语：我是 ZWM 2.0，今日所有目标皆已实现，身心合一，能量满格，从容掌控人生游戏\n3. 执行要求：全程不碰手机，仅专注画面与情绪',
    reminder: {
      enabled: true,
      time: '07:00',
  ...
  },
  {
    ...
    logs: {},
    priority: 'low' as const,
    note: '1. 大声朗读 ZWM 2.0 核心肯定语：我是 ZWM 2.0，人生游戏的唯一开发者，主动迭代，绝不回滚，所有愿望皆在行动中实现\n2. 调动「已拥有一切」的积极情绪，保持饱满状态\n3. 执行要求：声音洪亮，情绪饱满，完成后立即进入首个时间盒任务',
    reminder: {
      enabled: true,
      time: '07:25',
      repeat: 'daily' as const
  ...
  },
  {
    ...
    logs: {},
    priority: 'low' as const,
    note: '1. 立刻停下手头所有事务，调整为 ZWM 2.0 挺拔体态\n2. 默念肯定语：此刻我是 ZWM 2.0，单线程专注，拒绝内耗，每一次行动都在推进项目的成果\n3. 关闭手机所有通知，重启任务倒计时\n4. 执行要求：快速完成，无拖延，立即回归核心任务',
    reminder: {
      enabled: true,
      time: '09:00',
      repeat: 'daily' as const
  ...
  },
  {
    ...
    logs: {},
    priority: 'low' as const,
    note: '1. 快速查看上午时间盒完成度，记录预估时长与实际耗时的偏差\n2. 确认核心任务推进进度，标记已完成的小节点\n3. 默念肯定语：我稳步推进目标，每一次复盘都在优化路径，上午的执行已为成功奠定基础\n4. 执行要求：简单记录，不纠结未完成项，聚焦优化方向',
    reminder: {
      enabled: true,
      time: '12:00',
      repeat: 'daily' as const
  ...
  },
  {
    ...
    logs: {},
    priority: 'low' as const,
    note: '1. 播放 ZWM 2.0 专属音乐，闭眼放松，感受身心舒适的状态\n2. 回忆上午执行任务时的掌控感，强化积极情绪\n3. 默念肯定语：我能量充沛，身心舒适，从容应对下午任务，所有目标都在按节奏落地\n4. 执行要求：全身心放松，为下午执行储备能量',
    reminder: {
      enabled: true,
      time: '12:05',
      repeat: 'daily' as const
  ...
  },
  {
    ...
    logs: {},
    priority: 'low' as const,
    note: '1. 汇总当日所有时间盒完成度，统计核心任务推进情况\n2. 标记当日已完成的小成果（如完成 1 项核心任务、破解 1 个卡点）\n3. 默念肯定语：我今日一丝不苟完成所有规划，日拱一卒，每一个完成的任务都是目标实现的实证\n4. 执行要求：简单记录，肯定当日成果，不内耗、不自我否定',
    reminder: {
      enabled: true,
      time: '18:00',
      repeat: 'daily' as const
  ...
  },
  {
    ...
    logs: {},
    priority: 'low' as const,
    note: '1. 躺在床上，闭眼预演 ZWM 2.0 明日任务推进的完整画面\n2. 感受「明日目标已实现」的从容、笃定情绪，调整为放松体态\n3. 默念肯定语：我今日已完成所有动作，旧版本已覆盖，绝不回滚，明日的 ZWM 2.0 将更高效，所有愿望皆会实现\n4. 执行要求：全程不碰手机，带着积极情绪入睡',
    reminder: {
      enabled: true,
      time: '22:55',
      repeat: 'daily' as const
...
export const INITIAL_PROJECTS: Project[] = [
  {
    id: 'p1', name: '数码博主养成', startDate: '2025-12-23', description: '创建并运营第一个粉丝量达到1万的数码账号', status: 'active' as const, logs: [], dailyFocus: {}, todayFocusMinutes: 0, fears: [], attr: AttributeType.WEALTH, priority: 'medium' as const,
    subTasks: [
  ...
  {
    id: 'p4', name: '小红书虚拟店铺运营', startDate: '2025-12-23', description: '运营虚拟资料店铺，销售虚拟资料产品', status: 'active' as const, logs: [], dailyFocus: {}, todayFocusMinutes: 0, fears: [], attr: AttributeType.WEALTH, priority: 'medium' as const,
    subTasks: [
