...
export const helpContent: Record<string, HelpContentItem> = {
  ...
  data: {
    ...
    usageMethods: '1. 进入设置中心，找到"数据管理"选项；2. 点击"导出数据"按钮将当前游戏数据备份到本地；3. 保存备份文件到安全位置，如云端存储或外部硬盘；4. 若需要恢复数据，点击"导入数据"按钮，选择之前的备份文件；5. 确认恢复操作，等待数据导入完成；6. 若需要重新开始游戏，点击"重置所有数据"按钮，注意此操作不可逆；7. 定期备份数据，确保游戏进度安全。\n\n【WebDAV云端备份教程】\n1. 进入设置中心，找到"数据管理"选项，切换到"云端备份"标签；2. 选择"WebDAV"作为云服务提供商；3. 输入WebDAV服务器地址（如：https://dav.jianguoyun.com/dav/）；4. 输入WebDAV服务器的用户名和密码；5. 点击"保存WebDAV配置"按钮；6. 点击"备份到WebDAV"按钮，将数据备份到云端；7. 若需要从WebDAV恢复数据，点击"从WebDAV恢复"按钮。\n\n【坚果云WebDAV配置指南】\n1. 服务器地址：https://dav.jianguoyun.com/dav/；2. 用户名：坚果云账号（通常是邮箱）；3. 密码：坚果云应用密码（不是登录密码）；4. 如何获取应用密码：设置 → 安全选项 → 应用密码；5. 确保已启用WebDAV服务。\n\n【百度网盘云端备份教程】\n1. 登录百度开放平台（https://developer.baidu.com/），创建一个应用；2. 在应用详情页的"安全设置"中，设置"OAuth授权回调"为：http://localhost:3000/baidu-netdisk-callback；3. 等待1小时让回调地址生效；4. 在设置中心，找到"数据管理"选项，切换到"云端备份"标签；5. 选择"百度网盘"作为云服务提供商；6. 输入百度开放平台上的AppKey和SecretKey；7. 点击"授权百度网盘"按钮，在新打开的页面中完成授权；8. 授权成功后，点击"备份到百度网盘"按钮，将数据备份到云端；9. 若需要从百度网盘恢复数据，点击"从百度网盘恢复"按钮。','updateTime': '2026-02-09'
  },
