...
export const helpContent: Record<string, HelpContentItem> = {
  ...
  shop: {
    ...
    coreRules: '商品分为多个功能分类，包括吃喝、个人形象、休闲娱乐、家居数码、会员/权益/充值等；实物类商品购买后永久拥有，存入军械库；权益类商品购买即消耗，代表获得一次享受的权利；金币仅通过完成任务获得；商品分类可通过分类按钮进行筛选，各按钮显示对应分类的商品数量；支持商品的添加、编辑和删除操作（仅管理模式）；社群类商品统一归类到会员/权益/充值分类。',
  ...
  'shop-product-intro': {
    ...
    coreRules: '1. 金币来源：仅通过完成任务、成就解锁、签到等系统行为获得；2. 商品分类：家居数码、个人形象、休闲娱乐、会员权益等；3. 商品类型：实物类（永久拥有）、权益类（一次性享受）、盲盒类（随机奖励）；4. 管理功能：仅在管理模式下可添加、编辑、删除商品。',
  ...
  'shop-categories': {
    ...
    coreRules: '1. 预设分类：家居数码、个人形象、休闲娱乐、会员充值、运动健康、服装礼品等；2. 分类筛选：点击分类按钮即可显示该分类下的所有商品；3. 数量显示：分类按钮显示该分类商品的数量；4. 自定义：可在管理模式下添加新的分类。',
  ...
  data: {
    ...
    coreRules: '支持将游戏数据导出为JSON文件备份；可从备份文件中恢复数据，回到之前的游戏状态；重置所有数据会清空所有游戏进度，包括经验、财富、任务记录等；数据操作不可逆，执行前需确认；备份文件包含所有游戏数据，建议定期备份；支持手动备份和自动备份（未来版本）；支持通过 WebDAV 进行云端备份。',
    usageMethods: '1. 进入设置中心，找到"数据管理"选项；2. 点击"导出数据"按钮将当前游戏数据备份到本地；3. 保存备份文件到安全位置，如云端存储或外部硬盘；4. 若需要恢复数据，点击"导入数据"按钮，选择之前的备份文件；5. 确认恢复操作，等待数据导入完成；6. 若需要重新开始游戏，点击"重置所有数据"按钮，注意此操作不可逆；7. 定期备份数据，确保游戏进度安全。\n\n【WebDAV 云端备份教程】\n1. 进入设置中心，找到"数据管理"选项，切换到"云端备份"标签；2. 输入 WebDAV 服务器地址（如：https://dav.jianguoyun.com/dav/）；3. 输入 WebDAV 服务器的用户名和密码；4. 如有需要，填写备份目录路径；5. 点击"保存配置"按钮；6. 点击"备份到WebDAV"按钮，将数据备份到云端；7. 若需要从 WebDAV 恢复数据，点击"从WebDAV恢复"按钮。\n\n【坚果云 WebDAV 配置指南】\n1. 服务器地址：https://dav.jianguoyun.com/dav/；2. 用户名：坚果云账号（通常是邮箱）；3. 密码：坚果云应用密码，不是登录密码；4. 获取应用密码路径：账户信息 → 安全选项 → 应用密码；5. 建议先在坚果云中创建对应的备份目录。',
